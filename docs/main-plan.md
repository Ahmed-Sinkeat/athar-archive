...
| **M6** | Downloads + cache | Implementation complete: open-without-Download stops on reader close and remains evictable; explicit Download resumes, verifies, completes and retains; configurable eviction, low-storage behavior and grouped notifications are wired. Real-device download, retention, process-death and fully-offline open pass. Final instrumented acceptance is pending only because HyperOS blocks installation of the separate test APK until “Install via USB” is enabled. Evidence: [`docs/android/m6-results.md`](android/m6-results.md). |
