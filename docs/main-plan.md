...
| **M6** | Downloads + cache | ✅ Open-without-Download stops on reader close and remains evictable; explicit Download resumes, verifies, completes and retains; configurable eviction, low-storage behavior and grouped notifications are wired. Real-device download, retention, process-death and fully-offline open pass; the complete 12-test data suite later passed on the Android 15 phone. Evidence: [`docs/android/m6-results.md`](android/m6-results.md). |
| **M7** | Search + library organization | ✅ Complete: FTS5 + rowid join, `normalizeWithMap` snippets on vocalised text, structured filters, tap-to-paragraph, and durable Read later/Reading/Finished shelves plus user collections. All 12 data tests and all four app UI tests pass on Android 15. Evidence: [`docs/android/m7-results.md`](android/m7-results.md). |
| **M7.1** | Interaction quality + performance baseline | Before adding annotations or audio, establish repeatable real-device feedback loops for cold start, navigation/tab changes, library and search interaction, reader/list scrolling, and text selection. Remove visible jank, rough gestures and confusing flows; lock the fixes behind regression checks and the applicable §17 budgets. |
