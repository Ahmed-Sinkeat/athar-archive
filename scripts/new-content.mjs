...
const TEMPLATES = {
  ...
  question: (t) => fm([...shared(t),
    ...
    `# audio: audio-id-here      # optional → معرّف صوتية (الجواب الصوتي)`,
  ]) + "\nنصّ الجواب بالعربية…\n",
  ...
  annotation: (t) => fm([...shared(t),
    `target_type: poem          # REQUIRED: book | poem | quran`,
    `target_id: target-id-here  # REQUIRED → معرّف المتن/المنظومة/السورة`,
    `anchor: v1                 # REQUIRED → v{n} لبيت، أو p{n}/{#id} لفقرة`,
    `kind: شرح                  # شرح | حاشية | إعراب | تخريج | تفسير`,
