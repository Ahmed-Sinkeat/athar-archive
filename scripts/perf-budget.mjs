...
const BUDGET = 228 * 1024; // bytes: render-critical CSS+JS per page (HTML/content excluded — real Arabic books run 1 MB+ of legit prose; NFR-01 = ship light code). 150→160 for mobile app chrome (tab bar + bottom sheets CSS); 160→164 when /benefits' library.ts grew for the حفظ/"المحفوظات" bookmark tab; 164→204 when the mobile app redesign (settings/drawer anchored dropdowns + per-category icons) landed across every page's shared Base.css, plus /benefits' own copy-button addition — heaviest page (benefits, its own scoped CSS+JS on top of the shared bundle) measured 200.5 KB. 204→212 for the 2026-07-17 UX batch (تسميع mode, lazy per-source tafsir + sheet download button, surah jump nav, cross-deploy soft-nav guard) — measured 204.6 KB. 212→220 for the multi-place bookmark feature (popover markup/CSS + marks.ts/reader.ts/library.ts growth) — heaviest page (benefits) measured 217.1 KB. 220→228 for the interaction-design layer (Claude Design الحركة والتفاعل: motion tokens, feedback shadows, switch toggles, mobile settings sheet — all in the shared global.css bundle) plus the hero simplification — heaviest page (benefits) measured 224.0 KB.
