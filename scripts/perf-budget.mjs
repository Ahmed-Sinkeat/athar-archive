...
const BUDGET = 244 * 1024; // bytes: render-critical CSS+JS per page (HTML/content excluded — real Arabic books run 1 MB+ of legit prose; NFR-01 = ship light code). 150→160 for mobile app chrome (tab bar + bottom sheets CSS); 160→164 when /benefits' library.ts grew for the حفظ/"المحفوظات" bookmark tab; 164→204 when the mobile app redesign (settings/drawer anchored dropdowns + per-category icons) landed across every page's shared Base.css, plus /benefits' own copy-button addition — heaviest page (benefits, its own scoped CSS+JS on top of the shared bundle) measured 200.5 KB. 204→212 for the 2026-07-17 UX batch (تسميع mode, lazy per-source tafsir + sheet download button, surah jump nav, cross-deploy soft-nav guard) — measured 204.6 KB. 212→220 for the multi-place bookmark feature (popover markup/CSS + marks.ts/reader.ts/library.ts growth) — heaviest page (benefits) measured 217.1 KB. 220→228 for the interaction-design layer (Claude Design الحركة والتفاعل: motion tokens, feedback shadows, switch toggles, mobile settings sheet — all in the shared global.css bundle) plus the hero simplification — heaviest page (benefits) measured 224.0 KB. 228→232 — the matn bullet/asterisk fix + h4-h6 heading styles (both shared global.css) pushed benefits past 228 (measured 228.2 KB) with no slack left for the next bundle-wide CSS change; small headroom bump, not a new feature. 232→236 — المفضلة (favorites): the star-toggle button CSS (EntityCard.astro) and its click/state logic (reader.ts) are both in the shared bundle every page loads, pushing benefits past 232 (measured 233.3 KB); bumped with ~2.7 KB headroom, not a new feature. 236→244 — new /adhkar page (own CSS+JS) plus shared-bundle growth from the كُناشة note popup (marks.ts), search rebuild, and reader UX pass (InlineSearch.astro, reader.ts, global.css) pushed every page up; heaviest page (adhkar, own scoped code atop the grown shared bundle) measured 241.1 KB.
const TEXT_FLOOR = 100;    // min visible chars rendered without JS
const JS_DRIVEN = new Set(["/search", "/graph"]); // JS-driven tool pages exempt from weight/text
